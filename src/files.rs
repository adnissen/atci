...
pub fn load_sorted_paginated_cache_data(
    ...
    let total_pages = if limit > 0 {
        total_records.div_ceil(limit) // Ceiling division
    } else {
        1
    };
    ...
    let video_infos: Vec<VideoInfo> = all_entries
        ...
        .filter_map(|(entry, watch_directory)| {
            ...
            let (length, source) = if transcript_exists {
                ...
                (
                    metadata.clone().unwrap().length.clone(),
                    metadata.clone().unwrap().source.clone(),
                )
            } else {
                (None, None)
            };
...
pub fn web_get_files(
    ...
    if let (Some(page), Some(page_size), Some(sort_by), Some(sort_order)) =
        (page, page_size, sort_by.as_deref(), sort_order)
    {
        // Use the new paginated and sorted method with filtering
        match load_sorted_paginated_cache_data(
            filter_vec.as_ref(),
            page,
            page_size,
            sort_by,
            ...
            Ok(cache_data) => Json(ApiResponse::success(
                serde_json::to_value(cache_data).unwrap_or_default(),
            )),
            Err(e) => Json(ApiResponse::error(format!(
                "Failed to load paginated cache data: {}",
                e
            ))),
        }
    } else {
...
pub fn web_get_sources(_auth: AuthGuard) -> Json<ApiResponse<serde_json::Value>> {
    match load_cache_data() {
        Ok(cache_data) => Json(ApiResponse::success(
            serde_json::to_value(cache_data.sources).unwrap_or_default(),
        )),
        Err(e) => Json(ApiResponse::error(format!(
