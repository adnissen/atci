defmodule Atci.Web.TranscriptController do
  use Atci.Web, :controller
  require Logger

  alias Atci.{PathHelper, VideoInfoCache, ConfigManager}
  ...
  def queue(conn, _params) do
    queue_status = Atci.VideoProcessor.get_queue_status()
      ...
      case params["watch_directories"] do
        ...
        watch_dirs_param ->
          ...
          configured_watch_dirs =
            Atci.ConfigManager.get_config_value("watch_directories") || []
                    ...
                    case File.write(temp_text_path, text, [:utf8]) do
                      :ok ->
                        ...
                        {[
                           ...
                           file_path,
                           "-vf",
                           "drawtext=textfile='#{temp_text_path}':fontcolor=white:fontsize=#{font_size}:x=(w-text_w)/2:y=h-th-10",
                           "-vframes",
                           "1",
                           "-q:v",
                           "2",
                      ...
                      case File.write(temp_text_path, text, [:utf8]) do
                        :ok ->
                          ...
                          {[
                             ...
                             file_path,
                             "-vf",
                             "drawtext=textfile='#{temp_text_path}':fontcolor=white:fontsize=#{font_size}:x=(w-text_w)/2:y=h-th-10,fps=10,scale=480:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse",
                             "-loop",
                             "0",
                             "-y",
                             temp_clip_path
  ...
  def watch_directory(conn, _params) do
    watch_directory = Atci.ConfigManager.get_config_value("watch_directory")
  ...
  def regenerate_meta(conn, %{"filename" => filename}) do
    ...
    if Enum.empty?(watch_directories) do
      ...
      case video_file do
        ...
        file_path ->
          Atci.VideoProcessor.add_to_queue(file_path, :length)
      ...
      video_path ->
        case txt_file_path do
          ...
          _ ->
            ...
            Atci.VideoProcessor.add_to_queue(video_path, :partial, %{time: time_str})
