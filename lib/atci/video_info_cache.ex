defmodule Atci.VideoInfoCache do
  use GenServer
  require Logger

  alias Atci.{ConfigManager, PathHelper}
  ...
  defp extract_source_from_meta(meta_path) do
    case Atci.MetaFileHandler.get_meta_field(meta_path, "source") do
      {:ok, source} -> source
      {:error, _} -> nil
    end
  end
        ...
        |> Enum.map(fn file_path ->
          case File.stat(file_path) do
            {:ok, stat} ->
                  ...
                  case File.read(txt_path) do
                    {:ok, content} ->
                      ...
                      case File.stat(txt_path) do
                        {:ok, txt_stat} ->
                          {line_count,
                           txt_stat.mtime |> Atci.Web.TranscriptHTML.format_datetime(),
                           source}
              ...
              length =
                if transcript_exists do
                  case Atci.MetaFileHandler.get_meta_field(meta_path, "length") do
              ...
              %{
                name: display_name,
                base_name: filename,
                created_at: stat.ctime |> Atci.Web.TranscriptHTML.format_datetime(),
