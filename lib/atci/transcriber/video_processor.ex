defmodule Atci.VideoProcessor do
  use GenServer
  require Logger

  alias Atci.{PathHelper, VideoInfoCache, TranscriptModifier, MetaFileHandler}
  ...
  ## Examples
      ...
      iex> Atci.VideoProcessor.remove_from_queue(job)
      :ok
  """
  ...
  ## Examples
      ...
      iex> Atci.VideoProcessor.reorder_queue(new_order)
      :ok
  """
  ...
  ## Examples
      iex> Atci.VideoProcessor.get_queue_status()
  ...
  ## Examples
      iex> Atci.VideoProcessor.process_video_file(%{path: "video.mp4", time: nil}, :all)
      :ok

      iex> Atci.VideoProcessor.process_video_file(%{path: "video.mp4", time: nil}, :length)
      :ok

      iex> Atci.VideoProcessor.process_video_file(%{path: "video.mp4", time: "01:30:45"}, :partial)
      :ok
  """
  ...
  ## Examples
      iex> Atci.VideoProcessor.convert_to_mp3("video.mp4")
      {:ok, "/tmp/video_12345.mp3"}

      iex> Atci.VideoProcessor.convert_to_mp3("not_video.txt")
  ...
  ## Examples
      ...
      iex> Atci.VideoProcessor.transcribe_audio("audio.mp3", "video.mp4")
      :ok

      iex> Atci.VideoProcessor.transcribe_audio("not_audio.txt")
      {:error, :invalid_file_type}
  """
  def transcribe_audio(path, video_path) do
    if String.ends_with?(path, ".mp3") do
      whispercli = Atci.ConfigManager.get_config_value("whispercli_path")
      model = Atci.ConfigManager.get_config_value("model_path")
    ...
    if String.ends_with?(video_path, PathHelper.video_extensions_with_dots()) do
      case get_video_length(video_path) do
        {:ok, length} ->
          ...
          case Atci.MetaFileHandler.update_meta_field(meta_path, "length", length) do
            :ok ->
      ...
      {:ok, content} ->
        ...
        case File.write(txt_path, final_content, [:utf8]) do
          :ok ->
            ...
            case Atci.MetaFileHandler.update_meta_field(
                   meta_path,
                   "source",
                   "subtitles"
                 ) do
